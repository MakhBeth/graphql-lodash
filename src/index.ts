...
function getLodashDirectiveArgs(node) {
  ...
  for (let directive of node.directives || []) {
    if (directive.name.value !== lodashDirectiveDef.name) continue;
    if (lodashNode) throw Error(`Duplicating "@_" on the "${node.name.value}"`);
    lodashNode = directive;
  }

  if (lodashNode === null) return null;
...
function normalizeLodashArgs(argNodes, args) {
  ...
  each(argNodes, (node, name) => {
    ...
    else orderedArgs[name] = argValue;
  });
  return orderedArgs;
}
...
function applyOnPath(result, pathToArgs) {
  ...
  function traverse(root, pathRoot) {
    ...
    if (Array.isArray(root)) return root.map(item => traverse(item, pathRoot));
    ...
    if (typeof root === "object") {
      ...
      for (const key in pathRoot) {
        ...
        if (changedValue === null || changedValue === undefined) continue;
...
function stripQuery(queryAST): DocumentNode {
  return visit(queryAST, {
    [Kind.DIRECTIVE]: node => {
      if (node.name.value === "_") return null;
    }
  });
}

export const lodashDirectiveAST: DocumentNode = parse(
  new Source(lodashIDL, "lodashIDL")
);
    ...
    visit(root, {
      enter(node) {
        ...
        if (node.kind === Kind.FRAGMENT_SPREAD) {
          ...
          if (!fragment) throw Error(`Unknown fragment: ${fragmentName}`);
          traverse(fragment);
        }
      },
      leave(node) {
